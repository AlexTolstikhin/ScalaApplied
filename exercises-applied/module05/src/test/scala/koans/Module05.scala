...
class Module05 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Multiply numbers") {
    var multiplier = 3
    def mult(a: Int): Int = a * multiplier
  ...
  test ("Multiply variable number of arguments") {
    ...
    def multiplyDoubles(args: Double*): Double = {
      @tailrec
      def accumFunc(doubles: List[Double], accum: Double): Double = {
        doubles match {
          case Nil => accum
          case x :: tail => accumFunc(doubles.tail, accum * x)
        }
      }
      accumFunc(args.toList, 1.0)
    }
  ...
  test ("Recurse with varargs") {
    ...
    def listOfLists(strings: String*): List[List[String]] = {
      ...
      else strings.toList :: listOfLists(strings.toList.tail: _*)
    }
